...
export class BedroomService {
    ...
    async addRoom(data: IBedroom, boss: BossService, bossId: string): Promise<IBedroom> {
        try {
            ...
            await this.deleteImage(data.photo)
            throw error
        }
    }

    async updateRoomStatus(roomNumber: number, status: string, boss: BossService, bossId: string): Promise<void> {
        try {
            ...
            const updatedRoom = await this.repository.updateRoomStatus(roomNumber, status)
            return updatedRoom
        } catch (error) {
            throw error
        }
    ...
    async getFreeRooms(): Promise<IBedroom[]> {
        try {
            return await this.repository.getAllRoomsByStatus("disponível")
        } catch (error) {
            throw error
        }
    }
